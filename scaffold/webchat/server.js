...
chatOps.mountChatOps(app, { requireAuth, modelRouting, cwd: AGENT_ROOT, audit: auditRecord });
  ...
  ws.on('message', (data) => {
    ...
    const _startTurn = () => { child = chatSession.runChatTurn(
      ...
      (code, stderr) => {
        ...
        try { const sid = chatSession.currentSessionId(path.join(AGENT_ROOT, 'state'), model); auditRecord(chatSession.turnRecord(who, { model, prompt, reply: finalText, sessionId: sid, turnIndex: chatSession.nextTurnIndex(path.join(AGENT_ROOT, 'state'), sid), durationMs: Date.now() - t0, rc: code, error: code !== 0 ? errText : '' })); } catch (e) { /* the chain never blocks a reply */ }
        finish();
      }
    );
    child.on('error', (e) => {
      ws.send(JSON.stringify({ type: 'error', text: 'Failed to start claude: ' + e.message }));
      try { const sid = chatSession.currentSessionId(path.join(AGENT_ROOT, 'state'), model); auditRecord(chatSession.turnRecord(who, { model, prompt, reply: '', sessionId: sid, turnIndex: chatSession.nextTurnIndex(path.join(AGENT_ROOT, 'state'), sid), durationMs: Date.now() - t0, rc: -1, error: 'spawn: ' + e.message })); } catch (e2) { /* ditto */ }
      finish();
    });
    };
