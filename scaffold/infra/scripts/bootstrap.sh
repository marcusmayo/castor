...
label = urllib.parse.quote("Castor:webchat")
print(secret, f"otpauth://totp/{label}?secret={secret}&issuer=Castor&algorithm=SHA1&digits=6&period=30")
PY
)
[ -n "$TOTP_SECRET" ] || die "TOTP secret generation failed"
log "TOTP secret generated — scan this QR in your authenticator app:"
python3 - "$TOTP_URL" <<'PY'
import sys
try:
    import qrcode
    q = qrcode.QRCode(border=1); q.add_data(sys.argv[1]); q.make()
    q.print_ascii(invert=True)
except Exception as e:
    print("   (QR render unavailable: %s — use the manual entry secret below)" % e)
PY
printf '   manual entry secret: %s\n' "$TOTP_SECRET"
